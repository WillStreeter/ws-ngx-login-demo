...
/**
 * Builds an object consisting of the base configuration provided by confg/seed.server-app-env.ts, the additional
 * project specific overrides as defined in server-app-env/project.server-app-env.ts and including the
 *  base environment server-app-env as defined in env/base.ts
...
export class TemplateLocalsBuilder {
  ...
  build() {
    const configEnvName = argv['env-server-app-env'] || argv['server-app-env-env'] || 'dev';
